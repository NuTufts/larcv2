// #ifndef __BatchFillerClusterMask_CXX__
// #define __BatchFillerClusterMask_CXX__
//
// #include "BatchFillerClusterMask.h"
//
// #include <random>
//
// namespace larcv {
//
//   static BatchFillerClusterMaskProcessFactory __global_BatchFillerClusterMaskProcessFactory__;
//
//   BatchFillerClusterMask::BatchFillerClusterMask(const std::string name)
...
//   {}
//
//   void BatchFillerClusterMask::configure(const PSet& cfg)
//   {
//     LARCV_DEBUG() << "start" << std::endl;
//     _caffe_mode = cfg.get<bool>("CaffeMode",false);
//     _clustermask_producer = cfg.get<std::string>("ClusterMaskProducer");
//
...
//     if (_crop_clustermask)
//       _cropper.configure(cfg);
//
//     LARCV_DEBUG() << "done" << std::endl;
//   }
//
//   void BatchFillerClusterMask::initialize()
//   {}
//
//   void BatchFillerClusterMask::_batch_begin_()
//   {
//     _mirrored.clear();
...
//   }
//
//   void BatchFillerClusterMask::_batch_end_()
//   {
...
//   }
//
//   void BatchFillerClusterMask::finalize()
//   { _entry_data.clear(); }
//
//   size_t BatchFillerClusterMask::set_clustermask_size(const EventClusterMask* clustermask_data)
//   {
//     auto const& clustermask_vv = clustermask_data->clustermask_array();
//     if (clustermask_vv.empty()) {
//       LARCV_CRITICAL() << "Input image is empty!" << std::endl;
//       throw larbys();
//     }
//     if (_slice_v.empty()) {
//       _slice_v.resize(clustermask_vv.size());
...
//
//     if (clustermask_vv.size() <= _max_ch) {
...
//       _rows = clustermask_vv.at(_slice_v.front()).meta().rows();
//       _cols = clustermask_vv.at(_slice_v.front()).meta().cols();
//     }
//     else {
//       LARCV_CRITICAL() << "Cropping here IS NOT SET UP" << std::endl;
//       throw larbys();
//       // gonna crop (if speicifed dim is smaller than image dim)
//       _rows = std::min( clustermask_vv.at(_slice_v.front()).meta().rows(), _cropper.rows() );
//       _cols = std::min( clustermask_vv.at(_slice_v.front()).meta().cols(), _cropper.cols() );
//     }
//
//     LARCV_INFO() << "Rows = " << _rows << " ... Cols = " << _cols << std::endl;
//
//     // Define caffe idx to ClusterMask idx (assuming no crop)
...
//   }
//
//   void BatchFillerClusterMask::assert_dimension(const EventClusterMask* image_data) const
//   {
//     auto const& image_v = image_data->clustermask_array();
...
//   }
//
//   bool BatchFillerClusterMask::process(IOManager& mgr)
//   {
//     LARCV_DEBUG() << "start" << std::endl;
//     auto image_data = (EventClusterMask*)(mgr.get_data("clustermask", _image_producer));
//     if (!image_data) {
//       LARCV_CRITICAL() << "Could not locate image data w/ producer name " << _image_producer << std::endl;
//       throw larbys();
//     }
//     if (image_data->clustermask_array().empty()) {
...
//
//     auto const& image_v = ((EventClusterMask*)image_data)->clustermask_array();
//
//     for (size_t ch = 0; ch < _num_channels; ++ch) {
//
//       size_t input_ch = _slice_v.at(ch);

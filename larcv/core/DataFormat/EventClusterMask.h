...
namespace larcv {
  ...
  template<> inline std::string product_unique_name<larcv::EventClusterMask>() { return "clustermask"; }
  template EventClusterMask& IOManager::get_data<larcv::EventClusterMask>(const std::string&);
  template EventClusterMask& IOManager::get_data<larcv::EventClusterMask>(const ProducerID_t);
  /**
     \class larcv::EventClusterMask
     \brief A concrete factory class for larcv::EventClusterMask
  */
  class EventClusterMaskFactory : public DataProductFactoryBase {
  public:
    /// ctor
    EventClusterMaskFactory()
    { DataProductFactory::get().add_factory(product_unique_name<larcv::EventClusterMask>(),this); }
    /// create method
